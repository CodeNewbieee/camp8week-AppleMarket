...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val list = mutableListOf(
            Merchandise(R.drawable.sample1,"산진 한달된 선풍기 팝니다","이사가서 필요가 없어졌어요 급하게 내놓습니다","대현동",1000,"서울 서대문구 창천동",13,25,false),
            Merchandise(R.drawable.sample2,"김치냉장고","이사로인해 내놔요\n 직접 가져가셔야합니다.","안마담",20000,"인천 계양구 귤현동",8,28,false),
        ...
        with(binding) {
            ...
            val adapter = MerchandiseAdapter(list)
            rvMainList.adapter = adapter.apply {
                ...
                itemClick =
                    object : MerchandiseAdapter.ItemClick {
                        override fun onClick(view: View, position: Int) {
                            ...
                            intent.putExtra(Constans.ITEM_INDEX,position)
            ...
            activityResultLauncher = registerForActivityResult(ActivityResultContracts.StartActivityForResult()){
                if(it.resultCode == RESULT_OK) {
                    ...
                    if(isLike) {
                        if(!list[itemIndex].isLike){
                            list[itemIndex].isLike = true
                            list[itemIndex].good += 1
                        }
                    } else {
                        if(list[itemIndex].isLike){
                        list[itemIndex].isLike = false
                        list[itemIndex].good -= 1
                        }
                    }
